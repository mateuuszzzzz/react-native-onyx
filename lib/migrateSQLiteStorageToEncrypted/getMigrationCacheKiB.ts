// A large page cache keeps the migration's working set in RAM instead of repeatedly spilling and
// re-encrypting pages (~2x faster at 300MB on a Cortex-A53). A negative PRAGMA cache_size is a KiB
// memory bound, so capping it to a fraction of free memory keeps the migration from over-allocating
// (SQLite spills to disk instead) — no separate soft_heap_limit needed.
const MAX_CACHE_KIB = 131_072; // 128MB — sweet spot; beyond it the one-time per-page crypto dominates
const MIN_CACHE_KIB = 20_000; // ~20MB floor + used when memory unknown; keep in sync with SQLiteProvider's PRAGMA cache_size=-20000
