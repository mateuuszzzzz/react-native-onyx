...
import {
  ...
  SQLITE_STORAGE_VERSION_MIGRATED,
} from "../storage/providers/SQLiteConstants";
 ...
 */
const migrateSQLiteStorageToEncrypted: MigrateSQLiteStorageToEncrypted = ({
  keyId,
}): MigrationResult => {
  ...
  try {
    ...
    const userVersion =
      targetDb.execute<UserVersionResult>("PRAGMA user_version;").rows?.item(0)
        ?.user_version ?? 0;
    ...
    if (userVersion < SQLITE_STORAGE_VERSION_MIGRATED) {
      ...
      try {
        ...
        const legacyPageCount =
          targetDb.execute<PageCountResult>("PRAGMA legacy.page_count;").rows?.item(0)
            ?.page_count ?? 0;
        const legacyPageSize =
          targetDb.execute<PageSizeResult>("PRAGMA legacy.page_size;").rows?.item(0)
            ?.page_size ?? 0;
        ...
        const hasLegacyTable =
          (targetDb.execute(
            "SELECT 1 FROM legacy.sqlite_master WHERE type = 'table' AND name = 'keyvaluepairs';",
          ).rows?.length ?? 0) > 0;

        targetDb.execute("BEGIN;");
        if (hasLegacyTable) {
          targetDb.execute(
            "INSERT OR IGNORE INTO keyvaluepairs SELECT * FROM legacy.keyvaluepairs;",
          );
        }
        targetDb.execute(
          `PRAGMA user_version = ${SQLITE_STORAGE_VERSION_MIGRATED};`,
        );
