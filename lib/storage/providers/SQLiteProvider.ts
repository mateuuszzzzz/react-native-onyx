 ...
 */
function generateJSONReplaceSQLQueries(
  key: string,
  patches: FastMergeReplaceNullPatch[],
): string[][] {
...
const provider: StorageProvider<NitroSQLiteConnection | undefined> = {
   ...
   */
  init(keyId) {
    provider.store = keyId
      ? open({ name: SQLITE_ENCRYPTED_DB_NAME, keyId })
      : open({ name: SQLITE_PLAINTEXT_DB_NAME });
  ...
  getItem(key) {
    ...
    return provider.store
      .executeAsync<OnyxSQLiteKeyValuePair>(
        "SELECT record_key, valueJSON FROM keyvaluepairs WHERE record_key = ?;",
        [key],
      )
      .then(({ rows }) => {
  ...
  multiGet(keys) {
    ...
    return provider.store
      .executeAsync<OnyxSQLiteKeyValuePair>(command, keys)
      .then(({ rows }) => {
        // eslint-disable-next-line no-underscore-dangle
        const result = rows?._array.map((row) => [
          row.record_key,
  ...
  setItem(key, value) {
    ...
    return provider.store
      .executeAsync(
        "REPLACE INTO keyvaluepairs (record_key, valueJSON) VALUES (?, ?);",
  ...
  multiSet(pairs) {
    ...
    const params = pairs.map((pair) => [
      pair[0],
      JSON.stringify(pair[1] === undefined ? null : pair[1]),
    ]);
    if (utils.isEmptyObject(params)) {
      return Promise.resolve();
    }
    return provider.store
      .executeBatchAsync([{ query, params }])
      .then(() => undefined);
  },
  multiMerge(pairs) {
    if (!provider.store) {
  ...
  getAllKeys() {
    ...
    return provider.store
      .executeAsync("SELECT record_key FROM keyvaluepairs;")
      .then(({ rows }) => {
        // eslint-disable-next-line no-underscore-dangle
  ...
  removeItem(key) {
    ...
    return provider.store
      .executeAsync("DELETE FROM keyvaluepairs WHERE record_key = ?;", [key])
      .then(() => undefined);
  },
  removeItems(keys) {
    if (!provider.store) {
  ...
  clear() {
    ...
    return provider.store
      .executeAsync("DELETE FROM keyvaluepairs;", [])
      .then(() => undefined);
  },
  getDatabaseSize() {
    if (!provider.store) {
    ...
    return Promise.all([
      provider.store.executeAsync<PageSizeResult>("PRAGMA page_size;"),
      provider.store.executeAsync<PageCountResult>("PRAGMA page_count;"),
      getFreeDiskStorage(),
